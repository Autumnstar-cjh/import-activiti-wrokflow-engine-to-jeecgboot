...
public class ActTaskController {
    ...
    public Result<Object> back(@ApiParam("任务id") @RequestBody String id,
                               @ApiParam("流程实例id") @RequestBody String procInstId,
                               @ApiParam("意见评论") @RequestBody(required = false) String comment,
    ...
    public Result<Object> pass(@RequestBody HashMap<String, String> map){
        String id = map.get("id");
        String procInstId = map.get("procInstId");
        String assignees = map.get("assignees");
        Integer priority = Integer.valueOf(map.get("priority"));
        String comment = map.get("comment");
        Boolean sendMessage = Boolean.valueOf(map.get("sendMessage"));
        Boolean sendSms = Boolean.valueOf(map.get("sendSms"));
        Boolean sendEmail = Boolean.valueOf(map.get("sendEmail"));
        if(StrUtil.isBlank(comment)){
            comment = "";
        }
        taskService.addComment(id, procInstId, comment);
    ...
    public Result<Object> doneList(String name,
        ...
        Map<String, String> userMap = allUser.stream().collect(Collectors.toMap(ComboModel::getUsername, ComboModel::getTitle , (value1, value2 )-> (String) value1));
        taskList.forEach(e -> {
            HistoricTaskVo htv = new HistoricTaskVo(e);
            // 关联委托人
